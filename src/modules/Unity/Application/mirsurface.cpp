...
mir::EventUPtr makeMirEvent(QHoverEvent *qtEvent, MirPointerAction action, qreal dpr)
{
    ...
    return mir::events::make_event(0 /*DeviceID */, timestamp, 0 /* mac */, mir_input_event_modifier_none, action,
                                   buttons, qtEvent->posF().x() * dpr, qtEvent->posF().y() * dpr, 0, 0, 0, 0);
}

mir::EventUPtr makeMirEvent(QWheelEvent *qtEvent, qreal dpr)
{
    ...
    return mir::events::make_event(0 /*DeviceID */, timestamp, 0 /* mac */, modifiers, mir_pointer_action_motion,
                                   buttons, qtEvent->x() * dpr, qtEvent->y() * dpr,
...
void MirSurface::mousePressEvent(QMouseEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, mir_pointer_action_button_down, dpr);
    m_surface->consume(*ev);
    event->accept();
}

void MirSurface::mouseMoveEvent(QMouseEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, mir_pointer_action_motion, dpr);
    m_surface->consume(*ev);
    event->accept();
}

void MirSurface::mouseReleaseEvent(QMouseEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, mir_pointer_action_button_up, dpr);
    m_surface->consume(*ev);
    event->accept();
}

void MirSurface::hoverEnterEvent(QHoverEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, mir_pointer_action_enter, dpr);
    m_surface->consume(*ev);
    event->accept();
}

void MirSurface::hoverLeaveEvent(QHoverEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, mir_pointer_action_leave, dpr);
    m_surface->consume(*ev);
    event->accept();
}

void MirSurface::hoverMoveEvent(QHoverEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, mir_pointer_action_motion, dpr);
    m_surface->consume(*ev);
    event->accept();
...
void MirSurface::wheelEvent(QWheelEvent *event, qreal dpr)
{
    auto ev = makeMirEvent(event, dpr);
    m_surface->consume(*ev);
    event->accept();
}
...
void MirSurface::touchEvent(Qt::KeyboardModifiers mods,
                            ...
                            qreal dpr)
{
    auto ev = makeMirEvent(mods, touchPoints, touchPointStates, timestamp, dpr);
    m_surface->consume(*ev);
}
