    ...
    if (m_consumesInput && m_surface && m_surface->live()) {
        if (type() == Mir::InputMethodType) {
            ...
            if (isMouseInsideUbuntuKeyboard(event)) {
                m_surface->mousePressEvent(event, devicePixelRatio());
            } else {
                event->ignore();
            }
        } else {
...
{
    if (m_consumesInput && m_surface && m_surface->live()) {
        m_surface->mouseMoveEvent(event, devicePixelRatio());
    } else {
        event->ignore();
    }
}
...
{
    if (m_consumesInput && m_surface && m_surface->live()) {
        m_surface->mouseReleaseEvent(event, devicePixelRatio());
    } else {
        event->ignore();
    }
}
...
{
    if (m_consumesInput && m_surface && m_surface->live()) {
        m_surface->hoverEnterEvent(event, devicePixelRatio());
    } else {
        event->ignore();
    }
}
...
{
    if (m_consumesInput && m_surface && m_surface->live()) {
        m_surface->hoverLeaveEvent(event, devicePixelRatio());
    } else {
        event->ignore();
    }
}
...
{
    if (m_consumesInput && m_surface && m_surface->live()) {
        m_surface->hoverMoveEvent(event, devicePixelRatio());
    } else {
        event->ignore();
    }
}
            ...
            Qt::TouchPointStates touchPointStates)
{
    ...
    m_surface->touchEvent(mods, touchPoints, touchPointStates, timestamp, devicePixelRatio());
